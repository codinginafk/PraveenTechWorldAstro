...
description: "Stay Fix Event - If GA4 Realtime shows users but standard reports are blank, the tag is probably firing, but GA4 has not processed the data yet, a filter is removing"
...
tags:
  ...
  - website
  - tutorial
seoTitle: "GA4 Shows Realtime Users but Standard Repor..."
...
Google Support says GA4 report data is usually processed within 24 hours, and acquisition source data can take up to 48 hours ([Google Support](https://support.google.com/analytics/answer/1008594?hl=en)). I treat that as the first checkpoint, not as a reason to panic.
...
The fix is methodical. Check the stream first, then event processing, then filters, then report settings. If you want setup help, my [Google Tag Manager WordPress install guide](https://praveentechworld.com/install-google-tag-manager-wordpress/) covers the clean tag route, and my [GA4 tracking verification guide](https://praveentechworld.com/verify-ga4-tracking/) shows how I check events before changing reports.
...
If the tag is not firing at all, start with the [Google Tag Manager not firing fix](https://praveentechworld.com/google-tag-manager-not-firing/) before changing GA4 report settings.
...
If requests are blocked by a cookie banner, browser setting, or plugin, you may need to adjust consent mode or your tag firing rules. For that setup path, the [GA4 internal traffic filter guide](https://praveentechworld.com/ga4-internal-traffic-filters/) is a better starting point than reinstalling GA4.
