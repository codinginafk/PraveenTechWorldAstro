...
description: "GA4 Realtime shows users but standard reports are blank? The tag is firing but data has not processed yet â€” here is how to fix event processing."
coverImage: "https://images.unsplash.com/photo-1487014679447-9f8336841d58?ixid=M3w5NjkzNDF8MHwxfHNlYXJjaHwxfHx3ZWJzaXRlLXNldHVwfGVufDB8fHx8MTc4MTczMTkwMXww&ixlib=rb-4.1.0&w=1200&h=600&fit=crop"
...
That is why a fresh website can show 2 active users in Realtime, but the Acquisition, Engagement, and Pages reports still say â€œNo data available.â€ The hit reached GA4, but the report layer has not accepted it yet or is hiding it.
...
If the property name says â€œDemo Account,â€ â€œOld Site,â€ or a different domain, you are in the wrong place. Go back to the property list on the left and select the correct GA4 property.
...
If your GA4 property is set to Pacific Time but your server and visitors are in India, your â€œTodayâ€ report may not match your local day yet. This does not mean the data is gone. It means the reporting day is cut at a different hour.
...
If requests are blocked by a cookie banner, browser setting, or plugin, you may need to adjust consent mode or your tag firing rules. For that setup path, the [GA4 traffic setup guide](/blog/ga4-traffic-looks-wrong-after-website-setup-check-consent-mode-filters-and-cross) is a better starting point than reinstalling GA4.
...
Then open **Preview**, load the homepage, and confirm the tag fires on **All Pages**. Do not create a second tag just to â€œbe safe.â€ Duplicate GA4 tags can inflate sessions and break event counts.
...
If Realtime shows users and `page_view` appears in Events â†’ check date range, Data filters, internal traffic rules, and report comparisons.

If Realtime shows users but Events is empty â†’ wait up to 24 hours after confirming the tag is correct.

If Engagement has data but Acquisition is blank â†’ wait up to 48 hours for source attribution.

If DebugView has no events â†’ fix the Google tag or GTM setup before changing GA4 reports.

If filters removed all traffic â†’ set internal traffic rules back to Testing, correct the IP rule, then wait for processed reports.

If the property is wrong, duplicate, or polluted â†’ create a clean web data stream first, and reset the GA4 property only as the last resort.
...
- [undefined](/blog/backlink-building-guide-for-new-websites-get-your-first-quality-links) â€” 
