...
I’m an IT Operations Lead at a mid‑size university’s tech support team. Every Friday, faculty emailed me a CSV of student grades. I had to stitch them together, rename files, run a sanity check, and email the compiled report. The process took about 30 minutes, involved copying files across SharePoint, and was prone to human error-missing a column here, a typo there. The manual steps created a bottleneck before the weekend, and I knew I could improve the process. I wanted a script that would sit in a cron job, fetch the CSV, apply a simple transformation, and push a clean report to the department’s Dropbox folder-all without me touching a keyboard.
...
The changes were minor but essential: I moved the pandas import, fixed the grouping logic, cleaned the data, and corrected the logging. The final script is 2.5 KB (a few bytes larger due to comments) and runs in roughly 0.7 seconds on a typical Linux VM for a 500‑row CSV.
...
The experiment taught me the value of version-controlling the prompt. I keep the exact prompt used in the repo as `prompt.txt`. That way, if I need to re‑run the workflow later, I can reproduce the same AI output and see what changed.
