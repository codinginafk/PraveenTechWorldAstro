...
> **Direct Answer (NVIDIA Event ID 13):** The `nvlddmkm.sys Event ID 13` error indicates that the NVIDIA kernel display driver reported an unhandled graphics exception or TDR (Timeout Detection and Recovery) event. The 4 fastest fixes are: (1) perform a clean GPU driver reinstall using DDU in Windows Safe Mode, (2) disable GPU/VRAM overclocks and factory power-limit increases in MSI Afterburner, (3) verify dedicated 8-pin PCIe power cables (avoid daisy-chained pigtails), and (4) set Power Management Mode in NVIDIA Control Panel to "Prefer maximum performance".
