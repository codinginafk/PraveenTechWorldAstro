...
The average webpage is projected to ship **2.3 MB** of data by 2026. My old site was above average: a React SPA with 87 npm dependencies, a Webpack bundle that took 14 seconds to build, and a Lighthouse score of 38 on mobile. I spent more time updating dependencies than writing content.

I completely overhauled it, rebuilding the site with just 4 vanilla HTML files, SQLite via WebAssembly for search functionality, and edge hosting. Total payload: **14 KB**. Total build time: zero. No framework, no bundler, no node_modules folder.
...
The reality is often overlooked: **a blog doesn’t necessarily require a framework**. A documentation site does not need a framework. A landing page does not need a framework. If your content is text and images, the HTML tag is the only framework you need.
...
Bloat isn’t inevitable, it’s a deliberate decision. Every dependency you add trades your visitors' bandwidth for your convenience. For a content site, that trade is not worth it.
