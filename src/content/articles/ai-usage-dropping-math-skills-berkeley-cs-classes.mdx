---
title: "Why Berkeley CS Classes Banned AI Chatbots (Step-by-Step Study)"
...
faq:
  - question: "Is this just AutoGPT? How is it different?"
    answer: "While AutoGPT was an early implementation of this pattern, it had a significant flaw: it utilized the GPT-4 context window as its memory, leading to exponential costs and context limits. The Berkeley approach, however, uses SQLite for external memory, allowing the agent to read and write to the database without inflating costs based on steps taken."
  - question: "Is the custom autonomous agent cheaper than ChatGPT Plus?"
    answer: "Yes, it's considerably cheaper. The Berkeley agent employs OpenRouter's gpt-4o-mini at $0.15/M input tokens. A typical algorithms problem set costs around $0.08 to solve, while a ChatGPT Plus subscription is $20/month. If 10 problem sets are solved per month, the agent will cost only $0.80, and all outputs are kept in SQLite for review."
  - question: "What happens if the autonomous agent generates incorrect code?"
    answer: "It happens, and the executor will retry with the error message, fixing about 70% of the failures. The remaining 30% will require human intervention. The key difference is that, when the agent fails, a detailed error trace is provided along with the generated code and test output. With ChatGPT, an incorrect answer might just be provided without much context."
  - question: "Can this autonomous agent run on a laptop?"
    answer: "Absolutely! The entire system can run on a 2020 MacBook Air. The LLM calls are remote, the code execution happens locally, and the SQLite database is just a file. No need for a GPU or cloud credits - Docker Desktop manages the sandbox."
  - question: "Does using an autonomous agent violate Berkeley's academic integrity policy?"
    answer: "That depends on the specific course policy. The students who developed this used it as a learning tool; they reviewed the output, understood the code, and could explain each decision made by the agent. This approach is quite different from simply copy-pasting ChatGPT's answers. Most professors can differentiate between automating the output and using a tool to generate starter code for review."
---
## 1. Breaking the AI Chatbox: Berkeley's Autonomous Agents
...
## 6. Step-by-Step: Build Your Own Agent Sandbox

If you're feeling inspired and want to replicate this Berkeley setup in under 100 lines of Python, here's a quick guide from our team.
...
What do you think? Would you consider building your own autonomous agent? Our team would love to hear your thoughts!

## References & Further Reading for Why Berkeley CS Classes Banned AI Chatbots (Step-by-Step Study)
- [Wikipedia: Artificial Intelligence](https://en.wikipedia.org/wiki/Artificial_intelligence)
- [UC Berkeley: Computer Science](https://eecs.berkeley.edu/)

---

### Related Guides
- [How to Write Emails with AI That Get Replied To (5 Steps)](/blog/how-to-use-ai-to-write-emails-that-get-replies-2026-guide)
