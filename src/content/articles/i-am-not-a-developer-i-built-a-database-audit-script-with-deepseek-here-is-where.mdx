...
description: "A non-developer IT Ops Lead used DeepSeek to build a database audit Python script. The exact prompt used, three failures the AI produced, and the working 20-line script that finally passed."
...
tags:
  ...
  - python
  - prompt-engineering
seoTitle: "Non-Developer Builds Database Audit with DeepSeek — Full Breakdown"
...
faq:
  ...
  - question: "How do I schedule this script?"
    answer: "Add a cron entry like `/7     /usr/bin/python3 /opt/scripts/db_audit.py --config /opt/configs/db_audit.json --output /var/log/db_audit_$(date +\%Y\%m\%d).json`. The script runs in under 3 seconds, so it fits comfortably within a weekly window.  ---  "
---
...
**GitHub link:** You can fork the repo and see the full script with comments and a sample config here: https://github.com/praveentechworld/db-audit-script
