...
coverImage: "https://images.unsplash.com/photo-1487014679447-9f8336841d58?ixid=M3w5NjkzNDF8MHwxfHNlYXJjaHwxfHx3ZWJzaXRlLXNldHVwfGVufDB8fHx8MTc4MTczMDkyMnww&ixlib=rb-4.1.0&w=1200&h=600&fit=crop"
...
Filters are another common cause. GA4 data filters can remove internal traffic, developer traffic, or traffic marked with a specific `traffic_type`. Googleâ€™s official [GA4 data filters documentation](https://support.google.com/analytics/answer/11593727) explains that data filters control which events are included in reports. Iâ€™ve seen setups where the office IP was added correctly, but the filter was too broad and removed visitors from the same network range.
...
Do not compare GA4 users directly with Google Search Console clicks. They measure different things. Search Console clicks count search result clicks, while GA4 users count people who reached the site and passed your tag, consent, and filter setup. If you need help with that comparison, read [Google Search Console Not Showing Data](/blog/google-search-console-not-showing-data-8-fixes).
...
Use Googleâ€™s Campaign URL Builder for paid and email links. Keep naming consistent. Do not mix `utm_source=facebook`, `utm_source=Facebook`, and `utm_source=Meta Ads` for the same channel.
...
If traffic dropped after a cookie banner launch â†’ check Consent Mode first.

If Realtime works but standard reports look low â†’ wait 24-48 hours, then check filters and attribution.

If office or VPN visits are missing â†’ check **Data filters** and internal IP rules.

If users split between your main site and checkout domain â†’ fix cross-domain tracking.

If Search Console clicks do not match GA4 users â†’ compare landing pages and sessions, not clicks against users.

If the GA4 tag is missing or firing twice â†’ fix the base install before changing advanced settings.
...
- [Website Setup](/category/website-setup) â€” Browse all guides in this category
...
- [undefined](/blog/ga4-data-delayed-or-missing-after-installation-check-measurement-id-consent-and-) â€” 
